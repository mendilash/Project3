...
public class Converter {
    ...
    */
    ...
    public static String csvToJson(String csvString) {
        ...
        try {
            ...
            while ((line = bufReader.readLine()) != null ) {
                String[] rec = parser.parseLine(line);
                rowHeaders.add(rec[0]);
                JSONArray row = new JSONArray();
                row.add(new Long(rec[1]));
                row.add(new Long(rec[2]));
                row.add(new Long(rec[3]));
                row.add(new Long(rec[4]));
                data.add(row);
            }
    ...
    private static boolean jsonIsEqual(Object a, Object b) {
        if (a instanceof JSONObject && b instanceof JSONObject) {
            return jsonObjectIsEqual((JSONObject) a, (JSONObject) b);
        } else if (a instanceof JSONArray && b instanceof JSONArray) {
            return jsonArrayIsEqual((JSONArray) a, (JSONArray) b);
        } else {
            return a.equals(b);
        }
    }
    private static boolean jsonObjectIsEqual(JSONObject a, JSONObject b) {
        for (Object k : a.keySet()) {
            String key = (String) k;

            if (!b.containsKey(key) || !jsonIsEqual(a.get(key), b.get(key))) {
                return false;
            }
        }
        return true;
    }
    private static boolean jsonArrayIsEqual(JSONArray a, JSONArray b) {
        int aSize = a.size();
        ...
        if (aSize != b.size()) {
            ...
            for (int i = 0, j = aSize; i < j; i++) {
                if (!jsonIsEqual(a.get(i), b.get(i))) {
                    return false;
                }
            }
            return true;
